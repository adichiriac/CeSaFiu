...
function CareerDetailScreen({ career, onBack, onSave, isSaved, onChoose, isChosen }) {
  ...
  return (
    <div className="scroll-y" style={{ position: 'absolute', inset: 0, paddingBottom: 100 }}>
      ...
      <div style={{ padding: 20, marginTop: 8, display: 'flex', flexDirection: 'column', gap: 10 }}>
        <button
          onClick={onChoose}
          className="btn btn-lg"
          style={{
            width: '100%',
            background: isChosen ? 'var(--green)' : 'var(--purple)',
            color: isChosen ? '#000' : '#fff',
          }}
        >
          {isChosen ? '✓ TRASEUL TĂU #1' : 'ALEGE CA #1'}
        </button>
        <button onClick={onSave} className="btn btn-lg" style={{ width: '100%', background: '#fff', color: '#000' }}>
          {isSaved ? '★ SALVAT — apasă să scoți' : '☆ SALVEAZĂ ÎN VIBE-URI'}
        </button>
        <div className="body-sm" style={{ color: 'var(--ink-soft)', textAlign: 'center', lineHeight: 1.4, marginTop: 4 }}>
          „Alege ca #1" = direcția spre care te uiți. „Salvează" = idei pe care le explorezi.
        </div>
      </div>
    </div>
  );
