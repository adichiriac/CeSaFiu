  ...
  return (
    <div className="scroll-y" style={{ position: 'absolute', inset: 0, paddingBottom: 100 }}>
      ...
      <div style={{ padding: '16px 20px 20px' }}>
        <div className="card card-pop" style={{ padding: 18, background: 'var(--purple)', color: '#fff', position: 'relative' }}>
          <button
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            {window.AvatarPreview ? (
              <window.AvatarPreview user={user || {}} size={56} />
            ) : (
              <div style={{
                width: 56, height: 56,
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div className="h-sm" style={{ color: '#fff' }}>{user?.name || 'Alex'}, cls. a XI-a</div>
              <div className="body-sm" style={{ opacity: 0.85 }}>{user?.email || 'București · profil real'}</div>
            </div>
          </div>
          {user?.interests && user.interests.length > 0 && (
            <div style={{ marginTop: 12, display: 'flex', flexWrap: 'wrap', gap: 6 }}>
              {user.interests.slice(0, 4).map((tag) => (
                <span key={tag} style={{
                  background: 'rgba(255,255,255,0.18)', border: '1.5px solid #000',
                  padding: '3px 8px', fontSize: 10, fontWeight: 700,
                }}>{tag}</span>
              ))}
              {user.interests.length > 4 && (
                <span style={{
                  background: '#000', color: 'var(--yellow)', border: '1.5px solid #000',
                  padding: '3px 8px', fontSize: 10, fontWeight: 700,
                }}>+{user.interests.length - 4}</span>
              )}
            </div>
          )}
