...
function ResultsScreen({ matches, onPickCareer, onRetake, onProfile, onShareCard, layout }) {
  if (layout === 'list') return <ResultsList matches={matches} onPickCareer={onPickCareer} onRetake={onRetake} onShareCard={onShareCard} />;
  return <ResultsHero matches={matches} onPickCareer={onPickCareer} onRetake={onRetake} onProfile={onProfile} onShareCard={onShareCard} />;
}

function ResultsHero({ matches, onPickCareer, onRetake, onProfile, onShareCard }) {
  const top = matches[0];
  ...
  return (
    <div className="scroll-y" style={{ position: 'absolute', inset: 0, paddingBottom: 100 }}>
      ...
      <div style={{ padding: '24px 20px 0' }}>
        ...
        {/* Viral share CTA — generate personal card */}
        {onShareCard && (
          <button
            onClick={onShareCard}
            style={{
              ...
              cursor: 'pointer', boxShadow: '4px 4px 0 var(--purple)',
              display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8,
            }}
          >
            <span style={{ fontSize: 18 }}>✦</span>
            FĂ-ȚI CARDUL · SHARE CU COLEGII
          </button>
        )}
...
function ResultsList({ matches, onPickCareer, onRetake, onShareCard }) {
  return (
