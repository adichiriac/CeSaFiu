...
// ── PATH DETAIL ──
function PathDetailScreen({ pathId, onBack, onChoose, isChosen }) {
  ...
  return (
    <div className="scroll-y" style={{ position: 'absolute', inset: 0, paddingBottom: 100 }}>
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 18 }}>
          ...
          <button
            ...
            style={{
              background: isChosen ? '#000' : '#fff',
              color: isChosen ? 'var(--green)' : '#000',
              fontSize: 18,
            }}
            aria-label={isChosen ? 'Scoate alegerea' : 'Alege traseul'}
          >
            {isChosen ? '★' : '☆'}
          </button>
        </div>
      ...
      <div style={{ padding: '0 20px' }}>
        ...
        {/* Choose-as-path CTA — single chosen path is the "main bet" the student
            is committing to in their profile. Re-tap to unchoose. */}
        <button
          onClick={onChoose}
          className="btn btn-lg"
          style={{
            width: '100%', marginTop: 24,
            background: isChosen ? 'var(--green)' : 'var(--ink)',
            color: isChosen ? '#000' : '#fff',
          }}
        >
          {isChosen ? '✓ ACESTA E TRASEUL TĂU' : 'ALEGE ACEST TRASEU'}
        </button>
        <div className="body-sm" style={{ color: 'var(--ink-soft)', textAlign: 'center', lineHeight: 1.4, marginTop: 8, marginBottom: 8 }}>
          Doar unul activ. Apasă din nou ca să-l scoți.
        </div>
      </div>
    </div>
  );
...
function UniDetailScreen({ uniId, onBack, onSave, isSaved }) {
  ...
  return (
    <div className="scroll-y" style={{ position: 'absolute', inset: 0, paddingBottom: 100 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 18 }}>
          ...
          <button
            ...
            style={{
              ...
              color: isSaved ? 'var(--yellow)' : '#000',
              fontSize: 18,
            }}
            aria-label={isSaved ? 'Scoate din salvate' : 'Salvează'}
          >
            {isSaved ? '★' : '☆'}
          </button>
        </div>
      ...
      <div style={{ padding: '0 20px' }}>
        ...
        {/* Save CTA at bottom — mirrors Career Detail's pattern.
            Star top-right stays as a quick-action alternative. */}
        <div style={{ marginTop: 28, paddingTop: 16, borderTop: '2px solid #000' }}>
          <button
            ...
            style={{
              width: '100%',
              background: isSaved ? 'var(--green)' : 'var(--purple)',
              color: isSaved ? '#000' : '#fff',
              display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8,
            }}
          >
            {isSaved
              ? <><span style={{ fontSize: 16 }}>★</span><span>SALVATĂ — APASĂ SĂ SCOȚI</span></>
              : <><span style={{ fontSize: 18 }}>+</span><span>ADAUGĂ LA URMĂRITE</span></>}
          </button>
          <div className="body-sm" style={{ color: 'var(--ink-soft)', textAlign: 'center', lineHeight: 1.4, marginTop: 8 }}>
            Apare în profilul tău, la „Universități urmărite".
          </div>
        </div>
      </div>
    </div>
