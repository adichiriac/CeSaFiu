...
function App() {
  ...
  const [user, setUser] = useState({ name: 'Alex', email: '', avatarKind: 'preset', avatarId: 'star', avatarUrl: null, interests: [] });
  ...
  const tabBarVisible = ['results', 'career', 'profile', 'browse', 'pathDetail', 'uniDetail', 'deepResults', 'shareCard', 'editProfile'].includes(route.name);
    ...
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 32, background: 'var(--paper-2)' }}>
      ...
      <div className="phone bg-noise paper-tex" style={{ zIndex: 1 }}>
        ...
        <div className="phone-stack" style={{ top: tweaks.showStatusBar ? 92 : 60, bottom: tabBarVisible ? 70 : 0 }}>
          ...
          {route.name === 'results' && (
            <ResultsScreen matches={matches} onPickCareer={handlePickCareer} onRetake={handleStart} onProfile={() => goto('profile')} onShareCard={() => goto('shareCard')} layout={tweaks.resultsLayout} />
          )}
          {route.name === 'shareCard' && (
            <ShareCardScreen
              matches={matches}
              answers={answers}
              onBack={() => goto('results')}
              onCompare={() => goto('publicCard')}
            />
          )}
          {route.name === 'publicCard' && (
            <PublicCardScreen
              matches={matches}
              answers={answers}
              onBack={() => goto('shareCard')}
      ...
      <TweaksPanel title="Tweaks">
        ...
        <TweakSection label="Quick jump">
          ...
          <TweakButton label="→ Edit profile" onClick={() => goto('editProfile')} />
          <TweakButton label="→ Share card (viral)" onClick={() => { if (Object.keys(answers).length === 0) { setAnswers({ q1: data.questions[0].options[0], q2: data.questions[1].options[0] }); } goto('shareCard'); }} />
          <TweakButton label="→ Public card (guess mode)" onClick={() => { if (Object.keys(answers).length === 0) { setAnswers({ q1: data.questions[0].options[0], q2: data.questions[1].options[0] }); } goto('publicCard'); }} />
        </TweakSection>
      </TweaksPanel>
    </div>
  );
